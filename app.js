...
document.addEventListener('DOMContentLoaded', () => {
            ...
            card.innerHTML = `
                ...
                <div class="food-card-body">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                        <span style="display: flex; align-items: center; gap: 3px; background: #FFF8E7; border: 1px solid #FFE082; border-radius: 6px; padding: 2px 8px; font-size: 12px; font-weight: 700; color: #B8860B;">
                            <i data-lucide="star" width="11" height="11" style="fill: #FFB300; color: #FFB300;"></i>
                            ${food.rating ? food.rating.toFixed(1) : '4.5'}
                        </span>
        ...
        btnSaveProfile.addEventListener('click', (e) => {
            ...
            if (nameInput) {
                ...
                if (newName) {
                    ...
                    document.getElementById('profile-lbl-fullname').textContent = newName;
    ...
    document.querySelectorAll('.btn-coba').forEach(btn => {
        btn.addEventListener('click', () => {
            const foodId = btn.getAttribute('data-food-id');
            addToCart(foodId);
    ...
    const renderExploreMenu = (cat) => {
        ...
        filtered.forEach(food => {
            ...
            const buttonContent = isAvailable ?
                ...
                '<i data-lucide="ban" width="18" height="18"></i>';
            const buttonTitle = isAvailable ? 'Tambah ke Keranjang' : 'Habis';
            const buttonClass = isAvailable ? 'explore-add-btn btn-add-explore-cart' : 'explore-add-btn disabled';
            const buttonDisabledAttr = isAvailable ? '' : 'disabled';
            ...
            card.innerHTML = `
                ...
                <div class="explore-food-body">
                    ...
                    <div class="explore-food-footer">
                        ...
                        <button class="${buttonClass}" data-food-id="${food.id}" title="${buttonTitle}" ${buttonDisabledAttr}>
                            ${buttonContent}
                        </button>
                    </div>
                </div>
        ...
        document.querySelectorAll('.btn-add-explore-cart').forEach(btn => {
            btn.addEventListener('click', () => {
                const foodId = btn.getAttribute('data-food-id');
                addToCart(foodId);
            });
